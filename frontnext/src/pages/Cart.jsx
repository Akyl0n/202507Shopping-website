...
const Cart = () => {
  ...
  const [selected, setSelected] = useState([]); // 选中商品id数组
  ...
  return (
    <div className={styles['cart-container']}>
      <div style={{padding:'0 0 8px 0'}}>
        ...
        <label style={{fontSize:'0.98rem',color:'#888',display:'flex',alignItems:'center',gap:6,cursor:'pointer',userSelect:'none',marginLeft:2}}>
          <input type="checkbox" checked={allSelected} onChange={e=>handleSelectAll(e.target.checked)} style={{marginRight:4}} />
          ...
          {cart.map(item => (
            <div className={styles['cart-card']} key={item.id}>
              ...
              <div className={styles['cart-card-body']}>
                <label style={{display:'flex',alignItems:'center',gap:4,marginRight:12,cursor:'pointer',userSelect:'none'}}>
                  <input type="checkbox" checked={selected.includes(item.id)} onChange={e=>handleSelect(item.id, e.target.checked)} />
                ...
                <div className={styles['cart-info']}>
                  ...
                  <div className={styles['cart-qty']}>
                    数量：
                    <button className={styles['cart-btn']} onClick={() => decreaseQty(item.id)} disabled={item.qty <= 1}>-</button>
                    <span className={styles['cart-qty-num']}>{item.qty}</span>
                    <button className={styles['cart-btn']} onClick={() => increaseQty(item.id)}>+</button>
                  </div>
                </div>
                <button className={styles['cart-remove']} onClick={() => removeFromCart(item.id)}>移除</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {cart.length > 0 && (
        <div className={styles['cart-checkout-bar']}>
          <button className={styles['cart-checkout-btn']} onClick={handleCheckout}>购买</button>
        </div>
      )}
    </div>
  );
